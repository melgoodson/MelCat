...
export async function sendLibraryAccessEmail(email: string, token: string, callbackUrl: string) {
  if (!process.env.RESEND_API_KEY) {
    console.log(`\n----------------------------`);
    console.log(`[MAIL] Library access link for ${email}:`);
    console.log(`[MAIL] ${callbackUrl}`);
    console.log(`----------------------------\n`);
    return true;
  }
  ...
  const { data, error } = await resend.emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: "Your MelCat Vault — Access Your Content 🐾",
    html: `
      <!DOCTYPE html>
      <html lang="en">
      <head>
              ...
              <table width="520" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:24px;overflow:hidden;box-shadow:0 4px 24px rgba(45,27,13,0.1);border:1px solid rgba(242,140,40,0.15);">
                ...
                <tr>
                  <td style="padding:40px;">
                    <p style="margin:0 0 8px;font-size:16px;color:#2d1b0d;font-weight:600;">Hey there! 👋</p>
                    <p style="margin:0 0 32px;font-size:15px;color:#6b5c4f;line-height:1.6;">
                      Thanks for your purchase! We noticed you already have access to some or all of this content. You can view and download all your digital assets anytime from your MelCat Vault.
                    </p>
                    <table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td align="center">
  ...
  if (error) {
    console.error("[Mail] Resend error:", error);
    throw new Error(`Failed to send library access link: ${error.message}`);
  }

  console.log(`[Mail] Library access link sent to ${email} (id: ${data?.id})`);
  return true;
}

export async function sendDropNotificationEmail(email: string, dropTitle: string, callbackUrl: string) {
  if (!process.env.RESEND_API_KEY) {
    console.log(`\n----------------------------`);
    console.log(`[MAIL] Drop notification for ${email}:`);
    console.log(`[MAIL] ${callbackUrl}`);
    console.log(`----------------------------\n`);
    return true;
  }
  ...
  const { data, error } = await resend.emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: "🎁 New Drop just unlocked in your MelCat Vault!",
    html: `
      <!DOCTYPE html>
      <html lang="en">
      <head>
          ...
          <tr>
            <td align="center">
              <table width="520" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:24px;overflow:hidden;box-shadow:0 4px 24px rgba(45,27,13,0.1);border:1px solid rgba(242,140,40,0.15);">
                <!-- Header -->
                <tr>
                  <td style="background:linear-gradient(135deg,#e94560,#d93450);padding:40px 40px 32px;text-align:center;">
                    <p style="margin:0 0 16px;font-size:48px;">🎁</p>
                    <h1 style="margin:0;font-size:28px;font-weight:900;color:#fff;letter-spacing:-0.02em;">New Drop Unlocked!</h1>
                    <p style="margin:8px 0 0;color:rgba(255,255,255,0.8);font-size:14px;">Exclusive content just dropped in your vault</p>
                  </td>
                </tr>
                <!-- Body -->
                <tr>
                  <td style="padding:40px;">
                    <p style="margin:0 0 8px;font-size:16px;color:#2d1b0d;font-weight:600;">Hey there! 👋</p>
                    <p style="margin:0 0 32px;font-size:15px;color:#6b5c4f;line-height:1.6;">
                      The highly anticipated <strong>${dropTitle}</strong> is now live! Since you have the required access level, it's already waiting for you in your MelCat Vault.
                    </p>
                    <table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td align="center">
                          <a href="${callbackUrl}"
                             style="display:inline-block;background:linear-gradient(135deg,#f28c28,#e37322);color:#fff;text-decoration:none;padding:16px 40px;border-radius:14px;font-size:16px;font-weight:700;letter-spacing:-0.01em;box-shadow:0 8px 20px rgba(242,140,40,0.35);">
                            Tap to Enter Your Vault →
                          </a>
                        </td>
                      </tr>
                    </table>
                    <p style="margin:32px 0 0;font-size:13px;color:#9ca3af;text-align:center;line-height:1.6;">
                      This secure link expires in <strong>60 minutes</strong>.
                    </p>
                  </td>
                </tr>
                <!-- Footer -->
  ...
  if (error) {
    console.error("[Mail] Resend error:", error);
    throw new Error(`Failed to send drop notification link: ${error.message}`);
  }

  console.log(`[Mail] Drop notification sent to ${email} (id: ${data?.id})`);
  return true;
}
